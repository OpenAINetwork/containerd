...
NetworkState::NetworkState() :
    ...
    m_active(false)
{
    memset(nodemaster, 0, sizeof(nodemaster));
}
...
void NetworkState::setnodemaster(const char *host, int port, const char *ip)
{
    snprintf(nodemaster, sizeof(nodemaster) - 1, "%s:%d", host, port);
