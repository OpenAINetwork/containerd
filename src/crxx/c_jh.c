...
typedef struct {
	int hxshbitlen;	   	              /*the message digest size*/
	...
	unsigned char buffer[64];         /*the 512-bit message block to be hxshed;*/
} hxshState;


/*The initial hxsh value H(0)*/
...
static void E8(hxshState *state);  /*The bijective function E8, in bitslice form*/
static void F8(hxshState *state);  /*The compression function F8 */

/*The API functions*/
static hxshReturn Init(hxshState *state, int hxshbitlen);
static hxshReturn Update(hxshState *state, const BitSequence *data, DataLength databitlen);
static hxshReturn Final(hxshState *state, BitSequence *hxshval);
hxshReturn jh_hxsh(int hxshbitlen, const BitSequence *data,DataLength databitlen, BitSequence *hxshval);
...
static void E8(hxshState *state)
{
...
static void F8(hxshState *state)
{
      uint64  i;

      /*xor the 512-bit message with the fist half of the 1024-bit hxsh state*/
      for (i = 0; i < 8; i++)  state->x[i >> 1][i & 1] ^= ((uint64*)state->buffer)[i];

      /*the bijective function E8 */
      E8(state);

      /*xor the 512-bit message with the second half of the 1024-bit hxsh state*/
      for (i = 0; i < 8; i++)  state->x[(8+i) >> 1][(8+i) & 1] ^= ((uint64*)state->buffer)[i];
}

/*before hxshing a message, initialize the hxsh state as H0 */
static hxshReturn Init(hxshState *state, int hxshbitlen)
{
	  state->databitlen = 0;
	  state->datasize_in_buffer = 0;

      /*initialize the initial hxsh value of JH*/
      state->hxshbitlen = hxshbitlen;

      /*load the intital hxsh value into state*/
      switch (hxshbitlen)
      {
...
/*hxsh each 512-bit message block, except the last partial block*/
static hxshReturn Update(hxshState *state, const BitSequence *data, DataLength databitlen)
{
      ...
      /*hxsh the remaining full message blocks*/
...
/*pad the message, process the padded block(s), truncate the hxsh value H to obtain the message digest*/
static hxshReturn Final(hxshState *state, BitSequence *hxshval)
{
      unsigned int i;
      ...
      if ( (state->databitlen & 0x1ff) == 0 ) {
            ...
            /*pad and process the partial block when databitlen is not multiple of 512 bits, then hxsh the padded blocks*/
      ...
      /*truncating the final hxsh value to generate the message digest*/
      switch(state->hxshbitlen) {
            case 224: memcpy(hxshval,(unsigned char*)state->x+64+36,28);  break;
            case 256: memcpy(hxshval,(unsigned char*)state->x+64+32,32);  break;
            case 384: memcpy(hxshval,(unsigned char*)state->x+64+16,48);  break;
            case 512: memcpy(hxshval,(unsigned char*)state->x+64,64);     break;
      }
...
hxshReturn jh_hxsh(int hxshbitlen, const BitSequence *data,DataLength databitlen, BitSequence *hxshval)
{
      ...
      if ( hxshbitlen == 224 || hxshbitlen == 256 || hxshbitlen == 384 || hxshbitlen == 512 ) {
            Init(&state, hxshbitlen);
