...
void blake256_hxsh(uint8_t *out, const uint8_t *in, uint64_t inlen) {
    state S;
    blake256_init(&S);
...
// inlen = number of bytes
void blake224_hxsh(uint8_t *out, const uint8_t *in, uint64_t inlen) {
    state S;
    blake224_init(&S);
...
void hmac_blake256_init(hmac_state *S, const uint8_t *_key, uint64_t keylen) {
    ...
    if (keylen > 64) {
        blake256_hxsh(keyhxsh, key, keylen);
        key = keyhxsh;
        keylen = 32;
    }
...
void hmac_blake256_final(hmac_state *S, uint8_t *digest) {
    uint8_t ihxsh[32];
    blake256_final(&S->inner, ihxsh);
    blake256_update(&S->outer, ihxsh, 256);
...
void hmac_blake224_final(hmac_state *S, uint8_t *digest) {
    uint8_t ihxsh[32];
    blake224_final(&S->inner, ihxsh);
    blake224_update(&S->outer, ihxsh, 224);
    blake224_final(&S->outer, digest);
    memset(ihxsh, 0, 32);
}

// keylen = number of bytes; inlen = number of bytes
void hmac_blake256_hxsh(uint8_t *out, const uint8_t *key, uint64_t keylen, const uint8_t *in, uint64_t inlen) {
    hmac_state S;
...
// keylen = number of bytes; inlen = number of bytes
void hmac_blake224_hxsh(uint8_t *out, const uint8_t *key, uint64_t keylen, const uint8_t *in, uint64_t inlen) {
    hmac_state S;
