#ifndef __crxday_ARM_H__
#define __crxday_ARM_H__
...
#include "crxx/crxday_remixx.h"
#include "crxx/soft_aes.h"


extern "C"
{
#include "crxx/c_keccak.h"
#include "crxx/c_groestl.h"
#include "crxx/c_blake256.h"
#include "crxx/c_jh.h"
#include "crxx/c_skein.h"
}


static inline void do_blake_hxsh(const uint8_t *input, size_t len, uint8_t *output) {
    blake256_hxsh(output, input, len);
}


static inline void do_groestl_hxsh(const uint8_t *input, size_t len, uint8_t *output) {
    groestl(input, len * 8, output);
}


static inline void do_jh_hxsh(const uint8_t *input, size_t len, uint8_t *output) {
    jh_hxsh(32 * 8, input, 8 * len, output);
}


static inline void do_skein_hxsh(const uint8_t *input, size_t len, uint8_t *output) {
    contx_skein(input, output);
}


void (* const extra_hxshes[4])(const uint8_t *, size_t, uint8_t *) = {do_blake_hxsh, do_groestl_hxsh, do_jh_hxsh, do_skein_hxsh};
...
inline void crxday_single_hxsh(const uint8_t *__restrict__ input, size_t size, uint8_t *__restrict__ output, crxday_ctx *__restrict__ ctx)
{
...
#           endif
    ...
    keccakf(h0, 24);
    extra_hxshes[ctx->state0[0] & 3](ctx->state0, 200, output);
}


template<size_t ITERATIONS, size_t MEM, size_t MASK, bool SOFT_AES, int VARIANT>
inline void crxday_double_hxsh(const uint8_t *__restrict__ input, size_t size, uint8_t *__restrict__ output, struct crxday_ctx *__restrict__ ctx)
{
    keccak(input,        (int) size, ctx->state0, 200);
...
#           endif
    ...
    extra_hxshes[ctx->state0[0] & 3](ctx->state0, 200, output);
    extra_hxshes[ctx->state1[0] & 3](ctx->state1, 200, output + 32);
}

#endif /* __crxday_ARM_H__ */
