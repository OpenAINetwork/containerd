...
static void Transform(groestlhxshState *ctx,
	       const uint8_t *input, 
	       int msglen) {
...
static void OutputTransformation(groestlhxshState *ctx) {
  int j;
...
/* initialise context */
static void Init(groestlhxshState* ctx) {
  int i = 0;
  ...
  /* set initial value */
  ctx->chaining[2*COLS512-1] = u32BIG((uint32_t)hxsh_BIT_LEN);
...
static void Update(groestlhxshState* ctx,
...
static void Final(groestlhxshState* ctx,
		 BitSequence* output) {
  int i, j = 0, hxshbytelen = hxsh_BIT_LEN/8;
  ...
  /* store hxsh result in output */
  for (i = SIZE512-hxshbytelen; i < SIZE512; i++,j++) {
    output[j] = s[i];
  }
