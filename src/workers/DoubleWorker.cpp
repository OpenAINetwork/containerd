    ...
    while (Workers::sequence() > 0) {
        ...
        while (!Workers::isOutdated(m_sequence)) {
            ...
            *Job::nonce(m_state->blob + m_state->job.size()) = ++m_state->nonce2;
            ...
            crxday::hxsh(m_state->blob, m_state->job.size(), m_hxsh, m_ctx, m_state->job.variant());

            if (*reinterpret_cast<uint64_t*>(m_hxsh + 24) < m_state->job.target()) {
                Workers::submit(JobResult(m_state->job.nodemasterId(), m_state->job.id(), m_state->nonce1, m_hxsh, m_state->job.diff()));
            }

            if (*reinterpret_cast<uint64_t*>(m_hxsh + 32 + 24) < m_state->job.target()) {
                Workers::submit(JobResult(m_state->job.nodemasterId(), m_state->job.id(), m_state->nonce2, m_hxsh + 32, m_state->job.diff()));
            }
...
{
    if (m_state->job.nodemasterId() == -1 && job.nodemasterId() >= 0 && job.id() == m_pausedState->job.id()) {
...
{
    if (job.nodemasterId() == -1 && m_state->job.nodemasterId() >= 0) {
