...
void Workers::printhxshr(bool detail)
{
    m_hxshr->print();
}
...
void Workers::setJob(const Job &job, bool dnxx)
{
    uv_rwlock_wrlock(&m_rwlock);
    m_job = job;

    if (dnxx) {
        m_job.setnodemasterId(-1);
    }
...
void Workers::onTick(uv_timer_t *handle)
{
    for (Handle *handle : m_workers) {
        ...
        m_hxshr->add(handle->threadId(), handle->worker()->hxshCount(), handle->worker()->timestamp());
    }

    if ((m_ticks++ & 0xF) == 0)  {
        m_hxshr->updateHighest();
    }
