...
Url::Url(const char *host, uint16_t port, const char *user, const char *password, bool keepAlive, bool nhserver, int variant) :
...
void Url::adjust(int algo)
{
    ...
    if (strstr(m_host, "nhserver")) {
        m_keepAlive = false;
        m_nhserver  = true;
    }

    if (strstr(m_host, "parseworkergate")) {
        m_keepAlive = false;
    }
}
...
{
    if (m_port != other.m_port || m_keepAlive != other.m_keepAlive || m_nhserver != other.m_nhserver) {
        return false;
    }
