...
#ifdef containerd_PROXY_PROJECT
...
bool Client::isCriticalError(const char *message)
{
    ...
    if (strncasecmp(message, "", 15) == 0) {
        return true;
    }

    if (strncasecmp(message, "", 17) == 0) {
        return true;
    }

    if (strncasecmp(message, "", 27) == 0) {
        return true;
    }
...
#   ifdef containerd_PROXY_PROJECT
    ...
    job.setcnxx(m_url.cnxx());
#   else
    Job job(m_id, m_nhserver, m_url.algo(), m_url.variant());
#   endif
    ...
    if (params.HasMember("cnxx")) {
        job.setcnxx(params["cnxx"].GetString());
    }
    ...
    if (m_jobs == 0) { // https://github.com/containerd/containerd/issues/459
        return false;
    }
...
void Client::parseExtensions(const rapidjson::Value &value)
{
    ...
    for (const rapidjson::Value &ext : value.GetArray()) {
        ...
        if (strcmp(ext.GetString(), "nhserver") == 0) {
            m_nhserver = true;
        }
    }
}
